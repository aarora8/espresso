...
class RNN(nn.Module):
    ...
    def forward(self, input, hidden):
        ...
        # output = self.softmax(output)  # we don't need softmax for a siamese network
...
@register_model('siamese_rnn')
class FairseqSiameseRNN(BaseFairseqModel):
    ...
    def build_model(cls, args, task):
        ...
        return FairseqSiameseRNN(
            rnn=rnn,
            input_vocab=task.source_dictionary,
        )

    def __init__(self, rnn, input_vocab):
        super(FairseqSiameseRNN, self).__init__()
    ...
    def forward(self, src_tokens, src_lengths):
        # src_tokens consists of two pairs:
        # (src_tokens[0], src_tokens[1]) is positive
        # (src_tokens[0], src_tokens[2]) is negative
...
@register_model_architecture('siamese_rnn', 'example_siamese_rnn_arch')
def example_siamese_rnn_arch(args):
